...
open class LedActor( name: String, val led: ILed) : ActorBasic(name) {
...
fun main() : Unit = runBlocking{
    ...
    led.turnOff()
    val ledActor = LedActor("ledActor", led )
    //MsgUtil.forward( LedMsg.startBlink.name, LedMsg.startBlink.cmd, led1 )
    delay(2000)
   //CONTROL
      for( i in 1..3 ) {
        delay(500)
        ledActor.forward("ledCmd", LedCmd.LedOn().toString(), ledActor)
        delay(500)
        ledActor.forward("ledCmd", LedCmd.LedOff().toString(), ledActor)
    }
